...
> **Recommended MCP server** for querying Unity Catalog tables populated by the RDS migration pipeline.

This is the **primary MCP server** in the Spark-ling project when your data lives in
Unity Catalog (post RDS-to-Databricks migration). While the original server (`mcp/`)
uses a Databricks SQL warehouse (JDBC), AWS Athena, or local PySpark to run SQL queries,
this server uses **Databricks Connect** to give AI assistants a full remote SparkSession —
enabling DataFrame operations, execution plan analysis, caching, and more.
...
│ ↕ Unity Catalog        │ ← sparkling.banking.* (RDS migration tables)
└────────────────────────┘
```
...
## Integration with the RDS Migration Pipeline

After running the Databricks Asset Bundle jobs in `pipelines/rds_data_migration_1/`,
these Unity Catalog tables are available for AI-assisted querying:

| Table | Description |
|-------|-------------|
| `dim_customer` | Customer master with SCD Type 2 history |
| `dim_account` | Account dimension (SCD Type 1) |
| `dim_branch` | Branch reference table |
| `fact_transaction` | All banking transactions |
| `fact_daily_balance` | End-of-day account balance snapshots |
| `rpt_monthly_txn_summary` | Monthly transaction volume per customer/type |
| `rpt_account_balance_snapshot` | Daily balance with VND tiering |
| `rpt_customer_segment_kpi` | Segment KPIs for management reporting |
| `rpt_channel_analysis` | Digital vs traditional channel mix |
| `rpt_dormant_watchlist` | Inactive accounts (SBV compliance) |
...
| Unity Catalog tables | ✅ via SQL | ✅ via SQL + DataFrame API |
...
- RDS migration pipeline completed (Unity Catalog tables populated)
...
DATABRICKS_SCHEMA=banking   # schema containing RDS-migrated tables
MCP_TRANSPORT=stdio
```
...
> **Important**: Use the full `.venv/bin/python` path — not just `python`.

### Cursor / Gemini (`.gemini/settings.json`)
...
### Query RDS-migrated reporting tables
```
query_sql("SELECT segment, total_customers, dormant_account_pct FROM rpt_customer_segment_kpi ORDER BY total_customers DESC")
```

### SQL query — monthly digital channel trend
```
query_sql("SELECT report_month, SUM(pct_of_total_txns) AS digital_share FROM rpt_channel_analysis WHERE digital_flag = true GROUP BY report_month ORDER BY report_month")
```
...
### DataFrame operation — filter high-value transactions
```
dataframe_operation(
  table_name="fact_transaction",
  operation="filter",
...
### Explain a query (performance analysis)
```
explain_sql("SELECT c.segment, AVG(a.current_balance) FROM dim_customer c JOIN dim_account a ON c.customer_id = a.customer_id WHERE c.is_current = true GROUP BY c.segment")
```
...
| Tables not found | Run the RDS migration pipeline first: `databricks bundle run rds_data_migration_1_job` |
